...
type MonthlyReader = Pick<AnalyticsRepository, "getOverview" | "getMonthlyReport" | "getMonthlyAggregates" | "getBreakdowns">;

function errorResponse(status: 400 | 500, code: ErrorCode, message: string): Response {
  return Response.json({ ok: false, error: { code, message } }, { status });
}

async function createReader(): Promise<MonthlyReader> {
