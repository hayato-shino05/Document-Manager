...
public partial class BulkDeleteView : UserControl
{
    private bool _syncing;

    public BulkDeleteView()
    {
        InitializeComponent();
        ...
        Loaded += (_, _) =>
        {
            ...
            {
                if (DataContext is BulkDeleteViewModel vm)
                {
                    vm.Initialize();
                    SyncComboBoxes(vm);
                    SyncDataGrid(vm);
    ...
    {
        if (sender is not BulkDeleteViewModel vm) return;
    ...
    private void SyncComboBoxes(BulkDeleteViewModel vm)
    {
        _syncing = true;
        try
        {
    ...
    private void SyncDataGrid(BulkDeleteViewModel vm)
    {
